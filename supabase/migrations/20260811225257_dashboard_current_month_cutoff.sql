...
create or replace function public.dashboard_spending_breakdown(
  p_month date,
  p_range_from date default null,
  p_range_to date default null,
  p_category_ids uuid[] default null,
  p_subcategories boolean default false
)
returns table (
  category_id uuid,
  category_name text,
...
create or replace function public.dashboard_monthly_review(p_month date)
returns table (
  month date,
  income numeric,
  expenses numeric,
...
as $$
  with membership as (
    ...
    left join public.transactions as transaction
      ...
      and transaction.occurred_on < case
        when month.month::date = date_trunc('month', current_date)::date then current_date + 1
        else (month.month + interval '1 month')::date
      end
