create function public.save_current_settings(profile_name text, household_name text, member_color text)
returns text
language plpgsql
security definer
set search_path = ''
...
revoke execute on function public.save_current_settings(text, text, text) from public, anon;
grant execute on function public.save_current_settings(text, text, text) to authenticated;
