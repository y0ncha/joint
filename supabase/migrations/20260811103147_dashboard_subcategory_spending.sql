drop function public.dashboard_spending(date, date, date);

create function public.dashboard_spending(p_month date, p_range_from date, p_range_to date, p_category_id uuid)
returns table (
  category_id uuid,
  category_name text,
  amount numeric
...
as $$
  ...
  select
    case when selected_category.id is null then category.id else subcategory.id end,
    case when selected_category.id is null then category.name else subcategory.name end,
  ...
  where selected_category.id is null
    or (category.id = selected_category.id and subcategory.id is not null)
  group by selected_category.id, category.id, category.name, subcategory.id, subcategory.name
  order by 3 desc, 2;
$$;

revoke execute on function public.dashboard_spending(date, date, date, uuid) from public, anon, authenticated;
grant execute on function public.dashboard_spending(date, date, date, uuid) to authenticated;
