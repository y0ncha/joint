...
| TASK-030 | Add or extend focused tests for `src/app/actions/transactions.ts`, `src/app/actions/categories.ts`, `src/app/actions/profile.ts`, `src/components/transaction-sheet.tsx`, `src/components/bills-groceries-dashboard.tsx`, `src/components/settings-save-control.tsx`, `src/app/(app)/settings/page.tsx`, `src/app/(app)/bills-groceries/page.tsx`, and `src/components/workspace-shell.tsx` and verify the approved behavior. | Complete | 2026-07-31 |

### Implementation Phase 5

- GOAL-005: Reduce technical debt, redundant code, and tangled structure across the repository, permitting deliberate breaking internal-contract changes while preserving the approved UI and financial/security invariants.
- DEPENDENCIES: TASK-030 MUST be `Complete`.
- SCOPE: Audit and simplify the whole repository, including application code, server actions, database migrations and tests, shared utilities, UI primitives, tooling, configuration, documentation, and dependencies. Do not limit findings or fixes to Bills & Groceries files or the current feature branch's changes.
- BREAKING-CHANGE POLICY: Remove obsolete internal APIs, compatibility layers, unused exports, stale tests, and redundant module boundaries in one atomic repository change. Update every in-repository caller, test, documentation reference, and generated type affected by each break. Schema changes require a new immutable migration and remain subject to the explicit `joint-dev` authorization and Phase 6 hosted-verification gates; never weaken RLS, authentication, validation, or financial invariants to simplify code.
- COMPLETION CRITERIA: A repo-wide `ponytail:ponytail-audit` report is triaged and high-confidence findings are implemented with a net complexity reduction; deliberate contract breaks have no stale callers or references; performance changes have before-and-after evidence; focused/full tests, lint, diff checks, and settled desktop/mobile browser comparisons prove the approved UI remains unchanged.
- UI KEEP-LIST: Preserve visible copy, DOM structure, semantic tokens and classes, interactions, accessibility semantics, keyboard behavior, responsive layouts, and chart output unless a separately approved visual checkpoint changes the contract. Internal APIs, module boundaries, tests, configuration, and schema contracts may change under the breaking-change policy.

| Task     | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Status   | Date       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- | ---------- |
| TASK-031 | Record the dirty-worktree baseline and current changed-file ownership before cleanup; do not overwrite unrelated or concurrent work. Capture the approved Bills & Groceries desktop/mobile states needed for settled before-and-after comparison.                                                                                                                                                                                                                                    | Complete | 2026-07-31 |
| TASK-032 | Run `ponytail:ponytail-audit` once across the entire repository and save its ranked `delete:`, `stdlib:`, `native:`, `yagni:`, and `shrink:` findings with estimated line and dependency reductions. Treat the report as analysis only: verify every candidate against source, CSS, configuration, runtime, tooling, tests, and all callers before editing. Reject only candidates that break the UI keep-list, financial/security invariants, or hosted-schema authorization gates. | Complete | 2026-07-31 |
| TASK-033 | Implement the verified audit findings as deliberate repository-wide breaking cleanup: delete dead code and compatibility layers, reuse existing helpers or platform features, consolidate duplicate logic, and restructure tangled modules around existing responsibilities. Update every caller, test, document, and configuration reference in the same change; do not add speculative abstractions, dependencies, or cosmetic architecture churn.                                 | Complete | 2026-07-31 |
| TASK-034 | Run a separate normal performance review because `ponytail:ponytail-audit` excludes performance. Measure the changed server queries and warmed browser interactions, trace the actual hot path, and retain only changes with reproducible before-and-after improvement; do not add caching, memoization, indexes, aggregate storage, or debouncing without evidence and a reviewed need.                                                                                             | Complete | 2026-07-31 |
| TASK-035 | Verify deleted-symbol/reference scans, focused tests for every touched behavior, `bun run lint`, `bun run test`, and `git diff --check` without running `bun run build`. Compare settled desktop/mobile Bills & Groceries and every other touched route against TASK-031, including keyboard access and responsive behavior, and report every breaking contract, net lines/dependencies removed, measured performance results, and any unverified gap.                               | Complete | 2026-07-31 |

#### Phase 5 audit report (2026-07-31)

- `delete:` Remove the optional static Bills & Groceries fixture fallback and make the server-projected data contract required. Test with explicit fixture data instead. [`src/components/bills-groceries-dashboard.tsx`](../../src/components/bills-groceries-dashboard.tsx).
- `shrink:` Replace five copies of the Bills & Groceries request parsing and data-loading path with one shared loader and one validated dynamic detail route. [`src/app/(app)/bills-groceries/`](<../../src/app/(app)/bills-groceries/>).
- `yagni:` Inline the one-caller `DashboardControls` wrapper into the Overview route. Its replacement is the existing `LedgerMonthSelector` and owned Sheet primitives. [`src/components/dashboard-controls.tsx`](../../src/components/dashboard-controls.tsx).
- `yagni:` Inline the one-caller `StatementImportSheet` wrapper into the Transactions route. Its replacement is the existing `StatementImportForm` and owned Sheet primitives. [`src/components/statement-import-sheet.tsx`](../../src/components/statement-import-sheet.tsx).
- `shrink:` Use the existing `DateRange` contract for Bills & Groceries date windows instead of a second identical range type. [`src/lib/date-range.ts`](../../src/lib/date-range.ts).
- `net:` approximately -275 production-source lines, -0 dependencies; the dashboard source is 5,151 bytes smaller before minification. The warmed desktop dashboard reload measured 1.1 seconds locally. No speculative runtime optimization was retained because the mobile reading included development recompilation and was not comparable.

### Implementation Phase 6

- GOAL-006: Document the durable mechanism and verify local, hosted development, and release boundaries separately.
- DEPENDENCIES: TASK-012, TASK-021, TASK-030, and TASK-035 MUST be `Complete`.
