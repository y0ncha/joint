...
This plan replaces the earlier prorated-ledger design with an `Essentials` analytics route. The ledger and shared balance remain exact snapshots of stored transaction amounts and posting dates. Bills additionally store an inclusive service period used only for prorated Essentials analytics. Groceries analytics use actual posting dates, one optional fixed monthly household budget, and the protected `Main run` and `Top-ups` subcategories.
...
- **REQ-015**: `households.groceries_monthly_budget` MUST be nullable, positive when present, limited to two decimal places, and editable by either household member.
- **REQ-016**: Settings MUST add an `Essentials` card between `Household` and `Account` containing one `Monthly groceries budget` field integrated into the existing shared Save, dirty-state, navigation-warning, success, and error behavior.
- **REQ-017**: Clearing `Monthly groceries budget` MUST persist `NULL`; an unset budget MUST hide the chart threshold and show a concise Settings prompt without blocking Essentials.
- **REQ-018**: Add `/essentials` to desktop and mobile primary navigation with the accessible label `Essentials`.
- **REQ-019**: The Essentials configuration popover MUST switch the monthly charts between `Past 12 months` and `Calendar year`, store the choice in `period=rolling|calendar`, and treat missing or invalid values as `rolling`.
...
- **REQ-027**: The `Daily groceries` chart MUST default to the current month, support either `groceryMonth=YYYY-MM` or the inclusive pair `groceryFrom=YYYY-MM-DD` and `groceryTo=YYYY-MM-DD`, and prefer a valid custom pair over a month.
- **REQ-028**: The daily chart MUST include every calendar date in the selected range, including zero-spend dates, and sum same-day transactions separately into `Main run` and `Top-ups`.
- **REQ-029**: The daily chart MUST use a stacked lollipop composition: narrow stacked color segments for Main Run and Top-ups plus a rounded total marker, with a zero-height baseline marker for a no-spend date.
- **REQ-030**: Long daily ranges MUST retain daily granularity and use horizontal scrolling rather than dropping dates or changing to weekly aggregation.
- **REQ-031**: All four charts MUST show labelled axes, exact ILS tooltips, visible text legends, non-color labels, keyboard-accessible chart layers, and an accessible table containing the same values.
- **REQ-032**: Desktop MUST render `Bills by month` and `Year-over-year` in one responsive row and `Groceries by month` and `Daily groceries` in the next; mobile MUST stack all four cards full-width in the same order.
- **REQ-033**: Essentials MUST use bounded household-scoped queries for only the displayed window, required prior-year window, and Bills service periods that overlap either window; it MUST NOT call the existing all-transactions `getDashboardData()` path.
...
- **CON-006**: Stop after the visual-only Essentials and billing-range checkpoint until the user explicitly approves the rendered desktop and mobile result.
...
- **PAT-002**: Keep analytics pure in `src/lib/essentials.ts`, data access bounded in `src/lib/essentials-data.ts`, and chart interaction inside one client component.
...
- GOAL-001: Establish the approved product contract and obtain visual confirmation of the final Essentials surfaces.
- DEPENDENCIES: None.
...
- GOAL-002: Reset financial data and establish protected Essentials schema contracts.
- DEPENDENCIES: TASK-003 and TASK-004 MUST be `Approved`.
- COMPLETION CRITERIA: Local migration and pgTAP verification prove the authorized reset, deterministic seeding, future provisioning, protected identities, budget permissions, service periods, RLS, and grants.
...
| TASK-010 | In the generated Essentials migration, add nullable `households.groceries_monthly_budget numeric(12,2)`, positive/two-decimal validation, and the member-authorized `save_current_settings` contract and verify owner and member may set or clear it while nonmembers fail.             | Planned |      |
| TASK-011 | In the generated Essentials migration, add `transactions_household_service_period_idx` on `(household_id, service_period_start, service_period_end)` where both period columns are non-null and verify the overlap query can use it.                                                    | Planned |      |
| TASK-012 | Extend `supabase/tests/shared_balance.sql` with reset-boundary, seed, protection, exact-child, future-household, budget, period, import, RLS, privilege, and trigger cases and verify the repository database test target passes locally.                                               | Planned |      |
| TASK-013 | Regenerate `src/lib/database.types.ts` from the locally migrated schema and verify its diff matches only the approved migration contract.                                                                                                                                               | Planned |      |

### Implementation Phase 3

- GOAL-003: Implement deterministic Essentials analytics and bounded server data access.
...
| Task     | Description                                                                                                                                                                                                                                                                                                | Status  | Date |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ---- |
| TASK-014 | Extend `categoryFromRow()`, `subcategoryFromRow()`, and `transactionFromRow()` in `src/lib/finance-types.ts` with system keys and service periods and verify `src/lib/finance-types.test.ts` maps every new field.                                                                                         | Planned |      |
| TASK-015 | Add `src/lib/essentials.ts` with UTC range parsing, integer-agorot daily allocation, monthly consolidation, YoY alignment, grocery monthly stacks, complete daily series, highest-spend defaulting, and exact URL-default helpers and verify every exported function is pure.                              | Planned |      |
| TASK-016 | Add `src/lib/essentials.test.ts` cases for the `₪100.00` 2026-07-31 through 2026-08-03 allocation, uneven remainders, leap day, 366 days, multiple Bills consolidation, rolling and calendar windows, YoY gaps, budget absence, zero-spend dates, same-day Groceries sums, and deterministic tie-breaking. | Planned |      |
| TASK-017 | Add `src/lib/essentials-data.ts` with `getEssentialsData()` that derives household scope, resolves built-ins, queries only required columns and date/period windows, and returns compact chart series without calling `getDashboardData()`.                                                                | Planned |      |
| TASK-018 | Add `src/lib/essentials-data.test.ts` proving household filters, posting-date Groceries bounds, overlapping Bills bounds, prior-year bounds, selected-column lists, empty states, and absence of an unbounded transactions query.                                                                          | Planned |      |
| TASK-019 | Extend `src/lib/validation.ts` with optional paired billing-period validation and optional Groceries budget validation and verify `src/lib/validation.test.ts` covers empty, positive, precision, reversed, malformed, and oversized values.                                                               | Planned |      |
| TASK-020 | Extend `src/lib/financial-report.test.ts` and `src/lib/dashboard-data.test.ts` with Bills-period fixtures and verify ledger, overview, custom-range, and shared-balance outputs remain based only on full source amounts and posting dates.                                                                | Planned |      |
| TASK-021 | Run `EXPLAIN (ANALYZE, BUFFERS)` against representative bounded Groceries and Bills overlap queries, retain only the planned index unless evidence requires another reviewed index, and record the plans in the task evidence.                                                                             | Planned |      |
...
| Task     | Description                                                                                                                                                                                                                                                                                                                                                                                                         | Status  | Date |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ---- |
| TASK-022 | Update `createTransaction()` and `updateTransaction()` in `src/app/actions/transactions.ts` to resolve the selected parent system key, require or clear service periods, preserve posting-date behavior and import provenance, and verify household-scoped failures return field errors.                                                                                                                            | Planned |      |
| TASK-023 | Update `src/app/actions/categories.ts` and category management components to expose appearance-only controls for protected rows, retain Bills child management, remove Groceries child creation/movement/deletion paths, and verify forged mutations fail safely at the database.                                                                                                                                   | Planned |      |
| TASK-024 | Add `src/components/groceries-budget-settings-control.tsx`, render the `Essentials` card between Household and Account in `src/app/(app)/settings/page.tsx`, and verify both roles can stage a positive amount or an empty value.                                                                                                                                                                                   | Planned |      |
| TASK-025 | Extend `saveSettings()` in `src/app/actions/profile.ts` and dirty tracking in `src/components/settings-save-control.tsx` to distinguish unchanged, set, and cleared budget states and verify one atomic save persists all staged fields.                                                                                                                                                                            | Planned |      |
| TASK-026 | Add the `Essentials` navigation item in `src/components/workspace-shell.tsx` and verify desktop/mobile ordering, active-path behavior, accessible labels, and 44px targets.                                                                                                                                                                                                                                         | Planned |      |
| TASK-027 | Replace fixture data in `src/app/(app)/essentials/page.tsx` with validated URL state and `getEssentialsData()` output and verify invalid parameters canonicalize to approved defaults without losing unrelated selections.                                                                                                                                                                                          | Planned |      |
| TASK-028 | Complete `src/components/essentials-dashboard.tsx` with shadcn Chart/Recharts stacked bars, grouped YoY bars, budget `ReferenceLine`, stacked lollipop composition, configuration Popover, bill selectors, custom Calendar range, tooltips, legends, and equivalent tables and verify every interaction updates the intended URL state.                                                                             | Planned |      |
| TASK-029 | Complete `src/components/transaction-sheet.tsx` billing-period persistence and errors and verify selecting away from Bills clears the range while the transaction date remains the ledger date.                                                                                                                                                                                                                     | Planned |      |
| TASK-030 | Add or extend focused tests for `src/app/actions/transactions.ts`, `src/app/actions/categories.ts`, `src/app/actions/profile.ts`, `src/components/transaction-sheet.tsx`, `src/components/essentials-dashboard.tsx`, `src/components/settings-save-control.tsx`, `src/app/(app)/settings/page.tsx`, `src/app/(app)/essentials/page.tsx`, and `src/components/workspace-shell.tsx` and verify the approved behavior. | Planned |      |
...
| Task     | Description                                                                                                                                                                                                                                                                                   | Status  | Date |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ---- |
| TASK-031 | Add `docs/architecture/essentials-analytics.md` with trust boundaries, reset, protected taxonomy, source-versus-analytics semantics, proration, bounded queries, budget persistence, URL state, chart accessibility, failure behavior, and non-goals and link it from `docs/architecture.md`. | Planned |      |
| TASK-032 | Update `docs/architecture/financial-model.md` and `docs/roadmap.md` to distinguish the implemented Essentials subset from future generalized budgets and obligations and verify neither document claims unverified delivery.                                                                  | Planned |      |
...
| TASK-034 | Verify desktop, mobile, keyboard-only, reduced-motion, empty, missing-budget, missing-YoY, long-range scrolling, transaction create/edit, both-role budget save, and ledger-isolation flows in the local browser.                                                                             | Planned |      |
...
- **DEP-004**: Local Supabase and the repository database test command MUST be available for destructive migration verification.
...
- **FILE-001**: `docs/plans/essentials-dashboard.md` — approved source plan and delivery status.
- **FILE-002**: `docs/design.md` — visible Essentials, Settings, transaction-range, chart, and accessibility contract.
- **FILE-003**: The migration created by `SUPABASE_TELEMETRY_DISABLED=1 supabase migration new essentials_dashboard` under `supabase/migrations/` — destructive reset and complete schema contract.
...
- **FILE-007**: `src/lib/essentials.ts` and `src/lib/essentials.test.ts` — pure analytics and URL defaults.
- **FILE-008**: `src/lib/essentials-data.ts` and `src/lib/essentials-data.test.ts` — bounded household data access.
...
- **FILE-011**: `src/app/(app)/essentials/page.tsx` and `src/app/(app)/essentials/page.test.tsx` — Essentials route and validated URL state.
- **FILE-012**: `src/components/essentials-dashboard.tsx` and `src/components/essentials-dashboard.test.tsx` — four charts, controls, responsive composition, and accessible tables.
...
- **FILE-018**: `src/components/workspace-shell.tsx` and `src/components/workspace-shell.test.tsx` — Essentials navigation.
- **FILE-019**: `docs/architecture/essentials-analytics.md`, `docs/architecture.md`, `docs/architecture/financial-model.md`, and `docs/roadmap.md` — implemented durable architecture and remaining roadmap.

## 6. Testing

- **TEST-001**: Run `bun run test -- src/lib/essentials.test.ts src/lib/essentials-data.test.ts src/lib/finance-types.test.ts src/lib/validation.test.ts` and verify pure analytics, bounded queries, mappings, and validation.
...
- **TEST-004**: Run `bun run test -- src/components/transaction-sheet.test.tsx src/components/essentials-dashboard.test.tsx src/components/settings-save-control.test.ts src/app/'(app)'/settings/page.test.tsx src/app/'(app)'/essentials/page.test.tsx src/components/workspace-shell.test.tsx` and verify UI, URL, Settings, navigation, chart, and accessibility behavior.
...
- **TEST-006**: Run `bun run test:db` with the configured development database URL and verify the authorized reset boundary, protected seeds, periods, budget permissions, RLS, grants, and trigger assertions.
...
- **TEST-008**: In a local browser, verify a `₪100.00` Bills transaction dated 2026-07-31 with service period 2026-07-31 through 2026-08-03 remains one `₪100.00` July ledger row while Essentials shows `₪25.00` in July and `₪75.00` in August.
- **TEST-009**: In a local browser, verify rolling/calendar switching, all/default/custom Bills selections, highest-spend YoY default, missing prior year, budget present/absent, current month/custom Groceries dates, every zero day, stacked daily values, and long-range horizontal scrolling.
...
- **RISK-004**: Daily ranges can become visually wide; horizontal scrolling preserves every approved date without lossy aggregation.
