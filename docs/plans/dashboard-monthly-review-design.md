...
1. Income, Outgoings, and Monthly balance as three equal cards.
2. Where your money went beside a static Budgets placeholder.
3. One full-width six-month trend for income, outgoings, and monthly balance.
...
- The regular month view shows the selected month's income, outgoings, and monthly balance (`income - outgoings`). Each card compares its value with the average of the preceding three months.
- A custom range shows the selected range's income, outgoings, and monthly balance (`income - outgoings`). Each card compares with the preceding three equivalent ranges.
- The trend always shows six complete monthly points ending with the selected month, or the month containing a custom range's end date. Its three series are monthly income, monthly outgoings, and monthly balance (`income - outgoings`); it does not show the cumulative shared balance.
- Budgets is a non-interactive placeholder and has no data or reporting-period semantics.
