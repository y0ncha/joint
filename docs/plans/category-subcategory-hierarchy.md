...
- **REQ-003**: Replace `public.transactions.category_id` with nullable `public.transactions.subcategory_id`; manual transactions require a subcategory and statement-import rows may remain uncategorized.
...
| TASK-008 | Add authenticated `createSubcategory`, `updateSubcategory`, and `archiveSubcategory` Server Actions in `src/app/actions/categories.ts` that scope all queries to the current household and revalidate `/`, `/transactions`, and `/categories`. | Completed | 2026-07-26 |
