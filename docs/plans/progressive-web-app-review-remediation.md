...
![Status: Ready%20for%20release](https://img.shields.io/badge/status-Ready%20for%20release-green)

Remediate the verified `fded418..a1cdd1c` branch gaps while preserving the two-release schema transition. Release 1 is deployed, joint-dev is migrated, authenticated browser proof is complete, and 565 local tests pass; Release 2 remains.
...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ | ---------- |
| TASK-001 | On a user-selected branch based on `origin/main`, make `src/lib/dashboard-read-model.ts` recognize only `PGRST202` or `42883` missing-function errors from `dashboard_spending`, `dashboard_balance`, and `dashboard_recent_activity`, and return an explicit `schema_transition` result verified by focused tests. | Complete                                         | 2026-08-12 |
| TASK-002 | Update the corresponding dashboard cards in `src/app/(app)/page.tsx` to render neutral `Updating dashboard…` content for `schema_transition` results while preserving every other error as a failed request.                                                                                                        | Complete                                         | 2026-08-12 |
| TASK-003 | Run focused dashboard tests and the full local validation suite, then release this app-only compatibility change before any feature-branch migration is allowed to reach production.                                                                                                                                | Complete — PR #47 and production run 31542425705 | 2026-08-12 |
...
| Task     | Description                                                                                                                                                                                                                                                                                                                                                         | Status   | Date       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------- |
| TASK-007 | Create `supabase/migrations/*_dashboard_review_read_remediation.sql` with `supabase migration new dashboard_review_read_remediation`, adding `balance_change_percentage` to `dashboard_summary`, creating `dashboard_spending_breakdown(date,date,date,uuid[],boolean)`, and dropping the obsolete four-argument spending, activity, and category-change functions. | Complete | 2026-08-12 |
| TASK-008 | Implement `dashboard_spending_breakdown` so category mode returns selected parent totals and subcategory mode returns selected eligible-parent children in one query, with null or wholly ineligible selections falling back to all eligible categories.                                                                                                            | Complete | 2026-08-12 |
...
| TASK-011 | Extend `supabase/tests/dashboard_read_projections.sql` to verify balance comparison, selected parent aggregation, selected subcategory aggregation, all-category fallback, tenant isolation, function privileges, and absence of dropped projections.                                                                                                               | Complete | 2026-08-12 |
...
| Task     | Description                                                                                                                                                                                                                                                                              | Status                                                                                                         | Date       |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------- |
| TASK-012 | After confirming `supabase/.temp/project-ref` is `magcvzqnwrwxkhtsfspg` and receiving writer approval, run linked history, dry-run, push, history recheck, pgTAP, advisors, and catalog checks against `joint-dev`.                                                                      | Complete                                                                                                       | 2026-08-12 |
| TASK-013 | Regenerate `src/lib/database.types.ts` from the migrated schema and require it to contain the new breakdown and balance fields while omitting `dashboard_balance`, `dashboard_spending`, `dashboard_recent_activity`, `dashboard_category_changes`, and monthly-review `shared_balance`. | Complete                                                                                                       | 2026-08-12 |
...
| TASK-015 | Verify authenticated desktop/mobile dashboard loading, category and subcategory selection, custom-range comparisons, trend strokes, eleven-Bill legends, back/forward URL behavior, and a clean browser console.                                                                         | Complete                                                                                                       | 2026-08-12 |
| TASK-016 | Repeat correctness, architecture, ponytail, and canonical whole-repository security reviews after the encrypted subagent transport is restored, requiring no unresolved P0–P2 or medium-and-higher security finding.                                                                     | Complete — final parser findings closed with focused regressions; user waived a second whole-repository rescan | 2026-08-12 |
| TASK-017 | Release the schema and final application only after TASK-003 and TASK-012 through TASK-016 pass, keeping all production actions under explicit user control.                                                                                                                             | Ready for approved integration and Release 2                                                                   | 2026-08-12 |
...
- **RISK-004**: The final parser fixes have focused regression, lint, and type evidence; at the user's direction, the whole-repository security scan was not repeated after those fixes.
...
- [Dashboard monthly review design](dashboard-monthly-review-design.md)
- [Application runtime](../architecture/application-runtime.md)
- [Bills & Groceries analytics](../architecture/bills-groceries-analytics.md)
- [Joint design contract](../design.md)
- [Supabase database migrations](https://supabase.com/docs/guides/deployment/database-migrations)

## 9. Execution Evidence

- **EVID-001**: Local `bun run format:check`, `bun run lint`, `bun run typecheck`, `bun run test`, and `git diff --check` passed on 2026-08-12; Vitest reported 78 files and 565 tests passing.
- **EVID-002**: Hosted joint-dev pgTAP passes 17 dashboard assertions and 16 recurring-security assertions after linked history, dry-run, forward migration push, and history recheck.
...
- **EVID-004**: Release 1 shipped through PR #47 and production run 31542425705 before the dashboard-removal migration reached any release path. Generated database types now come from migrated joint-dev and omit obsolete projections.
- **EVID-005**: Focused regressions close recurring catch-up, ZIP decompression, sparse worksheet coordinates, merge expansion, XML collection amplification, and CSV row/column amplification before attacker-controlled data is materialized. The user accepted this targeted proof without another whole-repository scan.
- **EVID-006**: Authenticated in-app browser verification passed desktop/mobile layout, category/subcategory URL state and back/forward navigation, custom ranges, trend strokes, a 22-item Bills desktop legend, the independent mobile year-over-year legend, and clean consoles.
