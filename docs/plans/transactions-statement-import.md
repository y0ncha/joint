...
| TASK-024 | Add `supabase/migrations/<timestamp>_allow_member_card_mapping_updates.sql` through the Supabase CLI. Grant authenticated `UPDATE` on `public.member_card_mappings` and add a self-only policy with both `USING` and `WITH CHECK` predicates for `user_id = (select auth.uid())`; retain the existing household membership, SELECT, INSERT, uniqueness, and no-DELETE boundaries. | In progress — applied to joint-dev; pgTAP blocked by unavailable Docker | 2026-07-21 |
