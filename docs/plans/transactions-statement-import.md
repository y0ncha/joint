...
- **REQ-010**: Create an optional card-mapping setup page reachable only after successful household membership. OAuth redirects each newly joined or existing member to `/setup/card`, where the member may save one mapping or skip to `/`. The setup mutation MUST derive user and household identifiers from `requireCurrentHousehold()`.
- **REQ-011**: Members without a mapping MAY use the manual ledger and import statements. In Settings, a member MAY add or replace only their own four-digit mapping; an owner cannot manage another member's mapping. The updated mapping applies only to future statement imports. Existing `transactions.paid_by` values are never recomputed or changed by a mapping update.
...
- **SEC-002**: Enable RLS on the member-card mapping table. Household members may read mappings for their household; a member may insert or update only their own mapping after membership; no authenticated user may delete mappings or change mapping ownership through the Data API.
...
- **ALT-009**: Delete and recreate a mapping to replace its suffix. Rejected because a direct self-only update is atomic and avoids a transient unmapped state.
...
- **FILE-014**: `supabase/migrations/<timestamp>_allow_member_card_mapping_updates.sql`, `src/app/(app)/settings/page.tsx`, and their tests — self-only replacement of a member card mapping.
...
- **TEST-003**: Database pgTAP tests prove mapping-table RLS, self-only setup and update, household uniqueness, import metadata consistency, manual category enforcement, imported null-category support, nullable payer attribution with non-null payer membership validation, idempotency, and cross-household isolation.
...
- **RISK-003**: A member can mistype their mapping. Mitigation: Settings lets only that member replace it, preserves household-wide suffix uniqueness, and states that only future imports use the replacement.
