...
- **REQ-014**: The import UI MUST NOT ask for a category or create a fallback category. After import, a member MAY individually assign, change, or clear a matching category from the transaction edit sheet. Category suggestions, merchant-to-category rules, and bulk categorization remain deferred.
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-016 | Create `src/app/(app)/setup/card/page.tsx` and `src/components/member-card-setup-form.tsx`. Require member context, redirect to `/` when a mapping already exists, and render one `FieldGroup` form for four digits with visible label, privacy copy, validation, pending state, live result, a skip action, and a 44px submit target. After successful insertion or skip, redirect to `/`. Use existing `Card`, `Field`, `InputOTP`, and `Button`; add no dependency. | Complete | 2026-07-22 |
| TASK-017 | Update `src/app/auth/callback/route.ts` and its tests. Only after `ensurePartnerMembership` returns `joined` or `existing`, redirect to `/setup/card`; the page allows card setup or skipping to `/`. Preserve unmatched sign-out, OAuth error handling, provider checks, and membership as the authorization boundary. | Complete | 2026-07-22 |
...
- **TEST-005**: Component/page tests prove post-membership setup redirects, upload/setup gating, accessible form states, Merchant presentation, `Uncategorized` fallback, and preserved keyboard behavior.
