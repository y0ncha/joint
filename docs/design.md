...
- Mobile navigation exposes Overview, Transactions, Analytics, and Settings as 44px icon links in the bottom bar.
- Desktop and mobile primary navigation include `/analytics` with the accessible label `Analytics`.
...
- The transaction `Sheet` uses one shared `Recurring schedule` section for create, regular manual income or expense edits, and linked recurring edits. A regular manual income or expense edit can opt into recurrence using the same cadence and interval field template as create and recurring edit; the selected regular row becomes occurrence zero, and the next repeat is calculated after its current posting date. The recurring section appears before `Note`, and one primary `Save changes` action remains at the bottom after `Note`; saving a recurring edit or conversion must not require a second schedule save. For a linked recurring row, pause/resume and confirmed stop are icon-only controls immediately beside the cadence selector. Icon-only controls have accessible names, visible focus, keyboard operation, tooltips or titles, and at least 44px mobile targets. Scope controls gate edits: `this` may change kind and posting date but must not submit cadence changes; `future` and `all` may change the cadence/template but must not change kind or existing posting dates; `future` affects only ungenerated occurrences and `all` affects linked generated rows plus the template. Desktop keeps this in the existing right-side Sheet; mobile uses the existing full-height Sheet with responsive wrapping and no new page, nested dialog, or separate recurring form.
...
- A selected Bills subcategory reveals a `Billing period` range control. It uses inclusive ISO dates, defaults to the transaction date, and clears when a non-Bills subcategory is selected; the period is for Analytics analytics only.
...
- Icons supplement visible text. Icon-only controls require an accessible label; lifecycle icon-only controls also provide tooltips or titles, without relying on them as the only meaning.
...
- The primary experience exposes that shared balance, categories, manual income and expenses, statement imports, monthly reporting, recent activity, partner access, and Analytics analytics.
...
- Analytics are protected expense categories. Bills transactions require an inclusive billing period for Analytics-only prorated analytics; the ledger, reports, and shared balance continue to use the exact stored amount and posting date. Groceries has the protected `Main run` and `Top-ups` subcategories and supports one optional fixed monthly household budget for its Analytics threshold.
