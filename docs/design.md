...
- Merchant-rule creation and editing use a literal match builder in the existing right-side `Sheet`: members choose exactly `Contains`, `Is exactly`, `Starts with`, or `Ends with`, then enter the literal merchant text before choosing the existing action outcome.
