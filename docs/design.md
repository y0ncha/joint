...
- Merchant-rule creation and editing use a literal match builder in the existing right-side `Sheet`: members choose exactly `Contains`, `Is exactly`, `Starts with`, or `Ends with` from an owned `Select` styled like the current Action dropdown, then enter the literal merchant text before choosing the existing action outcome.
...
- Household members may configure ordered merchant automations. A rule either replaces a matching merchant with one literal normalized name or assigns an eligible non-Bills subcategory or direct `Other` category. Both actions may apply to a new transaction, priority is visible and user-controlled, and existing transactions change only after preview and confirmation.
