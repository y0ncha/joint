...
- Household members may configure ordered automations. A rule either replaces a matching merchant with one literal normalized name, assigns an eligible non-Bills subcategory or direct `Other` category, or stages matching existing transactions for deletion. Conditions can match Merchant, Note, and numeric Amount, with an AND/OR connector on each row after the first and left-to-right evaluation. Normalization and category assignment may apply to a new transaction; deletion applies only through the existing-transaction preview and confirmation. Priority is visible and user-controlled.
