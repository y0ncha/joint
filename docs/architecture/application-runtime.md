...
- `src/lib/dashboard-read-model.ts` owns the dashboard's focused, RLS-scoped reads: controls, summary with comparison percentages, one `dashboard_spending_breakdown` RPC for category or subcategory totals, and the six-month monthly review. `getLedgerData` supplies bounded transaction and recurring-schedule data to the transactions route. Dashboard controls, metrics, spending, and trend render in independent Suspense regions; the metric cards always use the summary's comparison policy, while the monthly review supplies the trend.
