...
- The planned `src/app/manifest.ts` and install icons in `public/brand/` and `src/app/apple-icon.png` will be static, unauthenticated installation metadata once added. They will describe Joint's identity and presentation only; they will not contain session or household financial data.
