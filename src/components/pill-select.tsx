    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverTrigger asChild>
        <Button type="button" variant="outline" disabled={disabled} className="h-11 w-full justify-start rounded-xl" aria-label={ariaLabel} onKeyDown={(event) => { if (event.key.length === 1) { setQuery(event.key); setOpen(true); } }}>
          {selected ? <Badge variant="outline" color={selected.color} className={cn("max-w-full truncate", !selected.color && !selected.className && "border-muted-foreground/20 bg-muted text-muted-foreground", selected.className)}>{selected.label}</Badge> : <span className="text-muted-foreground">{emptyLabel}</span>}
        </Button>
      </PopoverTrigger>
        ...
        <div className="flex max-h-56 flex-col gap-1 overflow-y-auto">
          {visibleOptions.map((option) => (
            <Button key={option.value} type="button" variant="ghost" className="h-11 justify-start" onClick={() => select(option.value)}>
              <Badge variant="outline" color={option.color} className={cn("max-w-full truncate", !option.color && !option.className && "border-muted-foreground/20 bg-muted text-muted-foreground", option.className)}>{option.label}</Badge>
            </Button>
          ))}
