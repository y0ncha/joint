...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "grid min-w-32 items-start gap-1.5 rounded-lg border border-border bg-popover px-2.5 py-1.5 text-xs text-popover-foreground shadow-md",
        className,
      )}
    >
      ...
      {payload
        ...
        .map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={cn("flex min-w-0 items-center gap-1.5 whitespace-nowrap [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground")}
            >
