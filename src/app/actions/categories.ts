...
export async function createCategory(input: FormData): Promise<ActionResult> {
  ...
  const { error } = await household.supabase.from("categories").insert({ household_id: household.householdId, name: parsed.data.name, kind: parsed.data.kind });
...
export async function updateCategory(categoryId: string, input: FormData): Promise<ActionResult> {
  ...
  if (!parsed.success) return validationError(parsed.error.issues);
  const household = await requireCurrentHousehold();
  const { error } = await household.supabase.from("categories").update({ name: parsed.data.name, kind: parsed.data.kind }).eq("id", categoryId).eq("household_id", household.householdId);
